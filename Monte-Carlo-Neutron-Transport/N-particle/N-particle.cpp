...
int main()
{
    ...
    for (int g = 0; g < generations; ++g)
    {
        ...
        std::ofstream dataFile("fissions.csv", std::ios::app); // Save fission positions to a CSV file

        for (auto &n : population)
        {
            if (!n.alive)
                continue; // Skip dead neutrons
        ...
        { // Arbitrary threshold for supercriticality
